    ...
    do {
      ...
      if (row_to_clear != -1) {
        for (int r = row_to_clear; r > 0; r--) {
          auto source_begin = data.begin() + (r - 1) * COLS;
          auto source_end = data.begin() + r * COLS;
          auto dest_begin = data.begin() + r * COLS;
...
public:
  ...
  Game() : score(0), rng(std::random_device{}()), dist(0, 6), is_running(true) {
    init_terminal();
  ...
  void tick() {
    if (!board.check_collision(active_x, active_y + 1, active_piece)) {
      ...
      if (lines_cleared > 0) {
        score += score_table[lines_cleared];
      }
  ...
  void rotate_piece() {
    ...
    if (!board.check_collision(active_x, active_y, rotated)) {
